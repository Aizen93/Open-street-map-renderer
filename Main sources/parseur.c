...
void initStruct(xmlNodePtr noeud,Map *m) {
	...
	/*pour remplir la hashmap des tags*/
	 ...
	 if(strcmp((char *)node->name, "tag") == 0) {
      ...
      //recuperation des relations et initialisation des champs
      if (strcmp((char *)noeud->name, "relation") == 0) {
	Relation *relation = malloc(sizeof(Relation));
	relation->member = NULL;
	relation->nb_ways = 0;
	relation->tag = g_hash_table_new (g_str_hash,g_str_equal);

	relation->id =  (char *) (xmlGetProp(noeud, ( xmlChar *) "id"));
	relation->visible = (char *) (xmlGetProp(noeud, ( xmlChar *) "visible"));
	g_hash_table_insert(m->relations,relation->id,relation);

	/*pour parcourir les member => id des noeuds qui constituent la relation en question*/
	xmlNodePtr node;
	for (node = noeud->children; node != NULL; node = node -> next) {
	  if ( strcmp ((char *)node->name, "member") == 0 ) {
	    relation->member = g_list_append(relation->member,(char *) xmlGetProp(node, (const xmlChar *)"ref"));
	    relation->nb_ways += 1;
	  }
	}
	...
	for (node = noeud -> children; node != NULL; node = node -> next) {
	  if (strcmp((char *)node->name, "tag") == 0) {
	    char *key = (char *) (xmlGetProp(node, (xmlChar *)"k"));
	    char *value = (char *) (xmlGetProp(node, (xmlChar *)"v"));
	    g_hash_table_insert(relation->tag,key,value);
	    relation->nb_tags +=1 ;
	  }
	}
      }
...
Map * initMap(char *docname) {
    ...
    map->relations = g_hash_table_new (g_str_hash,g_str_equal);
...
void delete_map ( Map *map) {
  ...
  g_hash_table_remove_all(map->relations);
  free(map);
}
